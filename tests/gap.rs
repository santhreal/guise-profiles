...
use guise_profiles::{
    ALL_PROFILES, DEFAULT_STEALTH_PROFILE, NetworkOsCoherence, StealthProfile, infer_initial_ttl,
    os_network_coherence, profile_os_network_stack,
};
...
fn unknown_platform_stack_fails_closed_and_catalogue_avoids_it() {
    use guise_profiles::{UserAgentPlatform, os_network_stack};
...
fn ie11_keeps_legacy_accept_encoding() {
    use guise_profiles::{DEFAULT_ACCEPT_ENCODING, LEGACY_ACCEPT_ENCODING, profile_facts};
    for profile in ALL_PROFILES {
        ...
        assert_eq!(profile_facts(*profile).accept_encoding, expected, "{profile:?}");
    }
}
